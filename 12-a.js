  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    if (node === target) {
      ...
      if (!visited.has(node)) {
        ...
        // top
        let top = `${x},${y - 1}`;
        if (y > 0 && graph.get(top) - graph.get(node) <= 1) {
          nextQueue.push([top, distance + 1]);
        }
        // right
        let right = `${x + 1},${y}`;
        if (x < rowLength && graph.get(right) - graph.get(node) <= 1) {
          nextQueue.push([right, distance + 1]);
        }
        // bottom
        let bottom = `${x},${y + 1}`;
        if (y < lineLength && graph.get(bottom) - graph.get(node) <= 1) {
          nextQueue.push([bottom, distance + 1]);
        }
        // left
        let left = `${x - 1},${y}`;
        if (x > 0 && graph.get(left) - graph.get(node) <= 1) {
