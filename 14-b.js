...
// add the floor
let height = bounds.maxY + 2;
for (let i = bounds.minX - (height + 2); i < bounds.maxX + (height + 2); i++) {
  graph.set(`${i},${height}`, "#");
}
...
while (roomForSand) {
  ...
  if (graph.has(`${nextSand[0]},${nextSand[1]}`)) {
    ...
    graph.set(`${currentSand[0]},${currentSand[1]}`, "o");
    //new sand starts
  ...
  //console.log(graph);
  if (graph.has(`${startSand[0]},${startSand[1]}`)) {
    roomForSand = false;
  }
  //if (unitsOfSand > 100) roomForSand = false;
}
