...
const getManhattanDistance = (pos) => {
  return Math.abs(pos[0] - end[0]) + Math.abs(pos[1] - end[1]);
};
//console.log(getManhattanDistance([6, 4]));
//console.log(getPossibleDirections([6, 4], getWindsAtTime(18)), end);
...
while (queue.length) {
  ...
  for (let i = 0; i < queue.length; i++) {
    let [pos, timePast] = queue[i];
    if (pos[0] === end[0] && pos[1] === end[1]) {
      //stop
      minTime = Math.min(minTime, timePast);
      console.log(timePast);
    } else {
      const left = getManhattanDistance(pos);
      const alreadyThereAtThisTime =
        records.has(`${pos[0]},${pos[1]}`) &&
        records.get(`${pos[0]},${pos[1]}`).includes(timePast);
      const tooLate = timePast + left >= minTime;

      if (!tooLate && !alreadyThereAtThisTime) {
        if (records.has(`${pos[0]},${pos[1]}`)) {
          records.get(`${pos[0]},${pos[1]}`).push(timePast);
        } else {
          records.set(`${pos[0]},${pos[1]}`, [timePast]);
        }
        // move in possible directions or wait
        const dirs = getPossibleDirections(pos, winds);
        //console.log(left, record, minTime);
        dirs.forEach((dir) => {
          nextQueue.push([dir, timePast + 1]);
        });
      }
    }
  }
