...
class Monkey {
  ...
  inspectItemAndSelectMonkey(item) {
    ...
    if (inspected % this.targetLevel === 0) {
      return [inspected, this.targetMonkeyTrue];
    } else {
      return [inspected, this.targetMonkeyFalse];
    }
  }
}
...
allFileContents.split(/\r?\n/).forEach((line) => {
  if (cycle === 0) {
    ...
    monkeys.get(currentMonkey).addTargetLevel(target);
  } else if (cycle === 4) {
...
while (round < 20) {
  ...
  for (let i = 0; i < iterate.length; i++) {
    ...
    while (j > 0) {
      ...
      //console.log(key, item, newItem, newMonkey);
